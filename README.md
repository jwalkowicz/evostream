# EvoStream-NLP: Ewolucyjna Samoadaptacja Potoków NLP 🧬
...
1. [Przegląd Projektu (Overview)](#-przegląd-proektu-overview)
2. [Główne Tezy Badawcze (Research Theses)](#-główne-tezy-badawcze-research-theses)
3. [Architektura Systemu (System Architecture)](#-architektura-systemu-system-architecture)
4. [Struktura Repozytorium (Codebase Structure)](#-struktura-repozytorium-codebase-structure)
5. [Instalacja i Wymagania (Setup & Prerequisites)](#-instalacja-i-wymagania-setup--prerequisites)
6. [Szybki Start: Interaktywny Panel Demonstracyjny (Live Web UI)](#-szybki-start-interaktywny-panel-demonstracyjny-live-web-ui)
7. [Uruchomienie Potoku Strumieniowego (CLI Pipeline)](#-uruchomienie-potoku-strumieniowego-cli-pipeline)
8. [Reprodukcja Wyników i Wykresów do Pracy (Thesis Benchmarks)](#-reprodukcja-wyników-i-wykresów-do-pracy-thesis-benchmarks)

---

## 🔬 Przegląd Projektu (Overview)

System **EvoStream-NLP** realizuje nienadzorowaną, dwufazową klasteryzację ewoluujących strumieni dokumentów tekstowych w czasie rzeczywistym. Wykorzystuje:
...
- **Nienadzorowany detektor dryfu (Unsupervised Drift Detector):** Ciągłe monitorowanie degradacji spójności klastrów ($Z$-score na wskaźniku *Silhouette*) oraz wskaźnika proliferacji bufora obserwacji odstających ($R_{\text{outlier}}$).
- **Wielokryterialny algorytm ewolucyjny (NSGA-II):** Samoczynne wyznaczanie frontu Pareto (kompromis: *Jakość semantyczna* vs. *Złożoność obliczeniowa*) i automatyczna adaptacja hiperparametrów ($\epsilon, \mu, \lambda, \epsilon_{\text{offline}}$) w wyznaczonym geometrycznie **punkcie kolana (Knee Point)**.

---

## 🎯 Główne Tezy Badawcze (Research Theses)
...
| **Teza 3** | Istnieje stabilny kompromis Pareto między jakością a złożonością, stabilizujący się w punkcie kolana przy niskich wymiarach projekcji. | **POTWIERDZONA** | Zbieżność punktu kolana (Knee Point) w zakresie niskich wymiarów ($d=16-32$) przy minimalnym koszcie obliczeniowym. |
...
│                                      │     │  ──► Wyznaczenie Frontu Pareto & Knee     │
└──────────────────────────────────────┘     └───────────────────────────────────────────┘
```

---

## 📂 Struktura Repozytorium (Codebase Structure)
...
│   │   ├── evolution.py           # Wielokryterialny optymalizator NSGA-II i Knee Point
...
│   ├── exp_thesis_1_ipca.py       # Eksperyment dla Tezy 1 (IPCA vs Full SBERT)
│   ├── exp_thesis_2_drift.py      # Eksperyment dla Tezy 2 (Reaktywna adaptacja dryfu)
│   ├── exp_thesis_3_pareto.py     # Eksperyment dla Tezy 3 (Fronty Pareto i Knee Point)
...
## ⚙️ Instalacja i Wymagania (Setup & Prerequisites)
...
## 🖥️ Szybki Start: Interaktywny Panel Demonstracyjny (Live Web UI)
...
1. **▶️ Następna partia (Step Batch):** Realny napływ dokumentów ze strumienia, ich osadzanie SBERT i redukcja IPCA w czasie rzeczywistym.
...
3. **💥 Wywołaj dryf! (Inject Drift):** Przycisk natychmiastowego przełączenia domeny tematycznej (np. z *Nauka/Kosmos* na *Polityka/Bliski Wschód*).
4. **⚠️ Reaktywna detekcja:** Prezentacja, jak wskaźnik obserwacji odstających ($R_{\text{outlier}} > 30\%$) i spadek Silhouette natychmiast wykrywają zmianę rozkładu.
5. **🧬 Samoadaptacja NSGA-II:** Wizualizacja wygenerowanego **frontu Pareto** oraz automatycznego doboru optymalnego **punktu kolana (Knee Point)**.
6. **🎯 Tryby adaptacji:** Możliwość przełączania w locie pomiędzy adaptacją *In-Place* a przeładowaniem modelu *Hot-Swap*.
7. **📜 Wyniki do pracy:** Przegląd wykresów i tabel z eksperymentów do Rozdziału 4.

---

## 🚀 Uruchomienie Potoku Strumieniowego (CLI Pipeline)
...
## 📊 Reprodukcja Wyników i Wykresów do Pracy (Thesis Benchmarks)
...
### 3. Teza 3: Fronty Pareto i Stabilizacja Punktu Kolana (Knee Point)
```bash
...
\hline
\text{Full SBERT (384d)} & 384 & 26.19 \pm 177.40 & 1.00\times & 19.3\% & \text{N/A} & 567.0 \\
...
### Wyniki dla Tezy 2: Reaktywna Samoadaptacja do Dryfu (In-Place vs Hot-Swap)
$$\begin{array}{lccccc}
\hline
...
\text{Adaptacja In-Place} & 85.8\% & 61.5\% & 78.5\% & 150\text{ dokumentów} & \textbf{Stopniowy / lekki dryf} \\
\mathbf{Adaptacja Hot-Swap (NSGA-II) \star} & \mathbf{85.8\%} & \mathbf{72.6\%} & \mathbf{88.7\%} & \mathbf{0\text{ dok. (Natychmiastowy)}} & \textbf{Nagły / głęboki dryf} \\
\hline
\end{array}$$

> [!TIP]
> **Kluczowy Wniosek Teoretyczny dla Komisji:** 
> * **Stopniowy dryf (Gradual):** Modyfikacja wag w locie (*In-Place Parameter Update*) jest optymalna obliczeniowo i zachowuje ciągłość ewolucji mikroklastrów bez konieczności resetowania stanu.
> * **Nagły dryf (Abrupt):** Wymaga wdrożenia nowego modelu (*Window-Trained Hot-Swap*), ponieważ próba gwałtownej zmiany parametrów w locie łamie założenia geometryczne promienia $\epsilon$ i powoduje histerezę przestarzałych mikroklastrów (tzw. *ghost micro-clusters*).

### Wyniki dla Tezy 3: Punkt Kolana (Knee Point) na Frontach Pareto
$$\begin{array}{ccccccc}
\hline
\textbf{Wymiar ($d$)} & \textbf{Rozmiar Frontu} & \textbf{Jakość (Knee)} & \textbf{Złożoność (Knee)} & \mathbf{\epsilon^*} & \mathbf{\mu^*} & \mathbf{\lambda^*} \\
\hline
